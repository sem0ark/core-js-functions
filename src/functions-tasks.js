...
function logger(func, logFunc) {
  return (...args) => {
    const expression = `${func.name}(${args
      .map((v) => JSON.stringify(v))
      .join(',')})`;

    logFunc(`${expression} starts`);
    const result = func(...args);
    logFunc(`${expression} ends`);
    return result;
  };
}
...
function nxtCurry(fn, ...args) {
  if (args.length >= fn.length) return fn(...args);
  return (...rest) => nxtCurry(fn, ...args, ...rest);
}

function partialUsingArguments(fn, ...args) {
  if (args.length >= fn.length) return () => fn(...args);
  return nxtCurry(fn, ...args);
}
